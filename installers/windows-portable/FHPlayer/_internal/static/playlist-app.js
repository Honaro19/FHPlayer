...
async function checkBackend() {
  try {
    ...
    state.backendCapabilities = {
      execute: data.capabilities?.execute !== false,
...
function getNormalizedCapabilityList(source) {
  const rawCapabilities = Array.isArray(source?.fullFunctionNames)
    ...
    : Array.isArray(source?.capabilities)
      ? source.capabilities
      : Array.isArray(source?.shortFunctionNames)
        ? source.shortFunctionNames.map((value) => LOVENSE_SHORT_CAPABILITY_MAP[String(value || "").trim().toLowerCase()] || value)
...
function normalizeExecutionMode(mode) {
  if (mode === "lovense-rules" || mode === "shell") {
    return "lovense-live";
  }
